...
char *
b64_enc(const char *str, size_t n, size_t *outn)
{
	...
	for (i = 0, j = 0; i + 3 <= n; i += 3, j += 4) {
		...
		out[j + 1] = BASE64_CHARS[((str[i + 0] & 0x3) << 4) | (str[i + 1] >> 4)];
		out[j + 2] = BASE64_CHARS[((str[i + 1] & 0xf) << 2) | (str[i + 2] >> 6)];
		out[j + 3] = BASE64_CHARS[str[i + 2] & 0x3f];
	}
